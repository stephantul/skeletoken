...
def test_preprocessor_empty_sequence_is_token_with_prefixes() -> None:
    ...
    preprocessor = Preprocessor(initial_subword_prefix="▁", continuing_subword_prefix="##")
    assert preprocessor.preprocess("", had_initial_subword_prefix=True, empty_sequence_is_token=True) == ["▁"]
    assert preprocessor.preprocess("", had_continuing_subword_prefix=True, empty_sequence_is_token=True) == ["##"]
...
def test_preprocessor_from_model_byte_level_without_add_prefix_space(small_tokenizer: Tokenizer) -> None:
    """Test that initial_subword_prefix is forced to None for byte-level models without add_prefix_space."""
    model = TokenizerModel.from_tokenizer(small_tokenizer)
    model.pre_tokenizer = ByteLevelPreTokenizer(add_prefix_space=False, use_regex=True, trim_offsets=True)
    preprocessor = Preprocessor.from_tokenizer_model(model)
    assert preprocessor.initial_subword_prefix is None


def test_preprocessor_word_and_continuing_subword_prefix_interplay() -> None:
    """Test that had_initial_subword_prefix and had_continuing_subword_prefix compose: strip one marker, add the other."""
    ...
    assert preprocessor.preprocess("hello", had_initial_subword_prefix=True, had_continuing_subword_prefix=False) == [
        "▁hello"
    ]
    # Non-initial: the "▁" is stripped
    assert preprocessor.preprocess("hello", had_initial_subword_prefix=False, had_continuing_subword_prefix=False) == [
        "hello"
    ]
    # Non-initial: "▁" is stripped.
    assert preprocessor.preprocess("hello", had_initial_subword_prefix=False, had_continuing_subword_prefix=True) == [
        "##hello"
    ]
