...
def test_reshape_embeddings_remaps_and_swaps_tokenizer(small_bert_checkpoint_dir: str) -> None:
    ...
    ColBERT subclasses SentenceTransformer, so it hits the same sentence-transformers>=5
    read-only `.tokenizer` property as skeletoken.external.sentence_transformers.
    """
